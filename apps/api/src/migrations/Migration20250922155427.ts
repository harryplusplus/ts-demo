...
export class Migration20250922155427 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`create table "user" ("id" bigserial primary key, "uuid" uuid not null, "email" text not null, "password_hashed" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null);`);
    this.addSql(`alter table "user" add constraint "user_uuid_unique" unique ("uuid");`);
    this.addSql(`alter table "user" add constraint "user_email_unique" unique ("email");`);

    this.addSql(`create table "refresh_token" ("id" bigserial primary key, "token" text not null, "expires_at" timestamptz null, "created_at" timestamptz not null, "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, "user_id" bigint not null);`);
